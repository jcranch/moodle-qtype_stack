...
class stack_cas_castext_owl extends stack_cas_castext_block {
    ...
    public function clear() {
        ...
        $div = html_writer::div(
         html_writer::tag('canvas','',array('class' => 'owlcanvas',
                                            'id' => $canvasid,
                                            'style' => $style)),
         'owlbox',
         array('id' => $divid)
    ...
    public function validate(&$errors=array()) {
        ...
        if (!$widthend) {
            $valid = false;
            $errors[] = stack_string('stackBlock_owl_width');
        }
        if (!$heightend) {
            $valid = false;
            $errors[] = stack_string('stackBlock_owl_height');
        }
        if (!preg_match('/^[0-9]*[\.]?[0-9]+$/', $widthtrim)) {
            $valid = false;
            $errors[] = stack_string('stackBlock_owl_width_num');
        }
        if (!preg_match('/^[0-9]*[\.]?[0-9]+$/', $heighttrim)) {
            $valid = false;
            $errors[] = stack_string('stackBlock_owl_height_num');
        }
